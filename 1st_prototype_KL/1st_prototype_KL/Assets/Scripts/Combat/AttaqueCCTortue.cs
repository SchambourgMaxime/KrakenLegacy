...
public class AttaqueCCTortue : MonoBehaviour
{
    ...
    public void attack()
    {
        ...
        if(timeLeft < 0 || attaque)
        {
            ...
            { 
                timer = Random.Range(0.1f, 0.3f);
                GameObject  go = (GameObject)  (Instantiate(bubbleAttak, this.transform.position + this.transform.up, this.transform.rotation));
                go.GetComponent<BubbleLife>().setOwner(gameObject);
                numberBubble--;
            }
            if (numberBubble <= 0)
            {

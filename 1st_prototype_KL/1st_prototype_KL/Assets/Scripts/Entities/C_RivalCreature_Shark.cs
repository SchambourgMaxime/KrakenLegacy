...
public class C_RivalCreature_Shark : C_BaseGameEntity
{
    ...
    void Awake () {
        ...
        defaultState = C_RivalCreatures_WanderState_Shark.Instance;
    ...
    public void selectTarget(Collider other)
    {
        if(FSM != null)
        {
            ...
            if (otherTag == "Player" || (otherTag.Contains("_RivalCreature") && selfTag != otherTag))
            {
                if (currentState.GetType() == typeof(C_RivalCreatures_FightState_Shark) && attackers.Count > 1)
                {
                    setTarget(other.gameObject);
                    FSM.ChangeState(C_RivalCreatures_FleeState_Shark.Instance);
                }
                if (currentState.GetType() == typeof(C_RivalCreatures_HuntState_Shark) || currentState.GetType() == typeof(C_RivalCreatures_WanderState_Shark))
                {
                    ...
                    if (initiateSharkAttack(SharkThresholdCalculation(Mathf.Pow(selfPower, 2), Mathf.Pow(otherPower, 2))))
                    {
                        setTarget(other.gameObject);
                        FSM.ChangeState(C_RivalCreatures_FightState_Shark.Instance);
                    }
                    else
                    {
                        setTarget(other.gameObject);
            ...
            else if (((otherTag.Contains("Fish") && !other.gameObject.GetComponent<C_Fish>().getIsHidden()) || otherTag.Contains("Plant") || otherTag == "Orbs")/* && satiety < maxSatiety*/) // Remplacer par _Fish
            {
                ...
                if (currentState.GetType() == typeof(C_RivalCreatures_WanderState_Shark)/* || lifeRatio <= 0.5f*/)
                {
                    setTarget(other.gameObject);
                    FSM.ChangeState(C_RivalCreatures_HuntState_Shark.Instance);
                }
                if (currentState.GetType() == typeof(C_RivalCreatures_HuntState_Shark))
                {
    ...
    public float SharkThresholdCalculation(float selfPower, float ennemyPower)
    {
    ...
    public bool initiateSharkAttack(float threshold)
    {
    ...
    public Vector3 barycentre(C_RivalCreature_Shark fugitive, List<GameObject> attackers)
    {
    ...
    public Vector3 setFleePoint(C_RivalCreature_Shark rivalCreature)
    {
